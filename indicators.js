...
function getCandleIconSVG(patternType) {
    switch (patternType) {
        case 'HAMMER':
            return `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="12" y1="2" x2="12" y2="22"/><rect x="8" y="2" width="8" height="6" fill="currentColor" rx="1"/></svg>`;
        case 'ENGULFING_BULL':
            return `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="6" y1="8" x2="6" y2="18" stroke="#ef4444"/><rect x="4" y="10" width="4" height="6" fill="#ef4444"/><line x1="16" y1="2" x2="16" y2="22"/><rect x="13" y="4" width="6" height="15" fill="currentColor" rx="1"/></svg>`;
        case 'ENGULFING_BEAR':
            return `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="6" y1="8" x2="6" y2="18" stroke="#10b981"/><rect x="4" y="10" width="4" height="6" fill="#10b981"/><line x1="16" y1="2" x2="16" y2="22"/><rect x="13" y="4" width="6" height="15" fill="currentColor" rx="1"/></svg>`;
        case 'G_CROSS':
            return `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M3 18L21 6" stroke="#10b981"/><path d="M3 6L21 18" stroke="#3b82f6"/><circle cx="12" cy="12" r="2" fill="#f59e0b"/></svg>`;
        default:
            return '';
    }
}
...
function evaluateEntryStrategy(stock) {
    ...
    if (isOversold || candle.type === "BULLISH") {
        entryStatus = "ENTRY";
    }
    if (isGoldenCross && stock.power_score >= 6.5) {
        entryStatus = "BUY";
    }
...
// Generator Icon Rocket, Pasir, Wait
function renderStatusBadge(analysis) {
    ...
    if (analysis.entryStatus === "BUY") {
        ...
        icon = "⏳"; // Merepresentasikan ikon pasir
        text = "ENTRY";
        cssClass = "status-entry";
    }

    return `<div class="status-badge-col ${cssClass}">${icon} ${text}</div>`;
}
